...
total_pupae_summary <- ddply(subset(annotated_pupae_data, Driver=='C179-Gal4'), ~Gene+UAS, summarize,
...
dead_pupae_summary <- ddply(subset(annotated_pupae_data, Driver=='C179-Gal4'), ~Gene+UAS, summarize,
...
total.pupae.aov <- aov(Total.Pupae~Driver*UAS, data=annotated_pupae_data)
```
...
```{r pupae-summary-tables,echo=FALSE,results='asis', message=FALSE}
library(xtable)
print(xtable(total_pupae_summary, caption="Summary of the Number of Pupae by Driver and UAS for C179-GAL4", label="tab:total-pupae-summary"), include.rownames=F, type='html')
print(xtable(total.pupae.aov, caption="ANOVA for the Number of Pupae by Driver and UAS for C179-GAL4", label="tab:total-pupae-anova"), type='html')
```

```{r c179-total-pupae-barplot,fig=TRUE,echo=FALSE}
ymax <- max(total_pupae_summary$mean+total_pupae_summary$se, na.rm=T)
plot <- barplot(total_pupae_summary$mean,
        beside=T, las=1,
        ylim=c(0,ymax),
        names.arg=total_pupae_summary$UAS,
        col=total_pupae_summary$Gene,
        ylab="Total Number of Pupae",
        xlab="shRNA Clone",
        main="Total Number of Pupae Using the C179-GAL4 Driver")

legend("top", levels(total_pupae_summary$Gene), fill= palette()[1:length(levels(total_pupae_summary$Gene))], bty='n')
...
superpose.eb(plot, 
             total_pupae_summary$mean,
             total_pupae_summary$se)
```
...
ymax <- max(dead_pupae_summary$mean+dead_pupae_summary$se, na.rm=T)
plot <- barplot(dead_pupae_summary$mean,
        beside=T, las=1,
        ylim=c(0,ymax),
        names.arg=dead_pupae_summary$UAS,
        col=dead_pupae_summary$Gene,
        ylab="Percent of Dead Pupae",
        xlab="shRNA Clone",
        main="Percent of Dead Pupae Using the C179-GAL4 Driver")

legend("topleft", levels(dead_pupae_summary$Gene), fill= palette()[1:length(levels(dead_pupae_summary$Gene))], bty='n')

superpose.eb(plot, 
             dead_pupae_summary$mean,
             dead_pupae_summary$se)
```

```{r c179-dead-pupae-statistics, echo=FALSE}
#perform anova testing first for an effect of the driver, then for an effect of the shRNAs
pupae.aov <- aov(Dead.Pupae.pct~Driver*UAS, data=annotated_pupae_data)
pupae.aov.c179 <- aov(Dead.Pupae.pct~UAS, data=subset(annotated_pupae_data, Driver=='C179-Gal4'))
#did dunnet test vs the control
library(multcomp)
pupae.dunnett <- glht(pupae.aov.c179, linfct=mcp(UAS="Dunnett"))
...
print(xtable(dead_pupae_summary, caption="Summary of Percent of Dead Pupae by Driver and UAS for C179-GAL4", label="tab:dead-pupae-summary"), include.rownames=F, type='html')
print(xtable(pupae.aov, caption="ANOVA for Percent of Dead Pupae by Driver and UAS for C179-GAL4", label="tab:dead-pupae-anova"), type='html')
print(xtable(pupae.aov.c179, caption="ANOVA for Percent of Dead Pupae by UAS for C179-GAL4", label="tab:dead-pupae-anova-c179"), type='html')
print(xtable(pupae.dunnett.summary, caption="Dunnett's Test for the Percent of Dead Pupae Comparing to Control (C179-Gal4/+)", label="tab:dead-pupae-dunnett", digits=c(0,1,7)), type='html')
```
...
print(xtable(total.flies.aov, caption="ANOVA for the Number of Flies by Driver and UAS for C179-GAL4", label="tab:total-flies-anova"), type='html')
print(xtable(fly.dunnett.summary, caption="Dunnett's Test for the Number of Flies Comparing to Control (C179-Gal4/+)", label="tab:total-flies-dunnett", digits=c(0,1,7)), type='html')
```
...
#perform anova testing first for an effect of the driver, then for an effect of the shRNAs
total.pupae.aov_mef <- aov(Total.Pupae~Driver*UAS, data=annotated_pupae_data_mef)
```
...
print(xtable(total.pupae.aov_mef, caption="ANOVA for the Number of Pupae by Driver and UAS for Mef-GAL4", label="tab:total-pupae-anova-mef"), type='html')
```
