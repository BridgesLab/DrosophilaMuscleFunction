...
The three Raptor knockout strains all had much less progeny than would be expected with 24B-GAL4 driven expression.  The adjusted p-values for these effects are `r paste(driver.summarised.data[4:6,'padj'], collapse=", ")` respectively for shRNA's `r paste(driver.summarised.data[4:6,'UAS'], collapse=", ")`.
...
require(xtable)
print(xtable(gene.summarised.data, caption="Gene Level Summarised Data", label="tab:gene_summarised_data", digits = c(rep(0,8),5,5)), include.rownames=F, type='html')
...
print(xtable(fisher.c179.gender, caption="Fisher Tests Comparing Gender Effects on C157/Raptor shRNA Crosses", label="tab:179-fisher", digits = c(0,4,4)), type='html')
...
print(xtable(summarised.data[order(summarised.data$Cross),], caption="Cross Level Summarised Data", label="tab:summarised_data", digits = c(rep(0,7),5,5)), include.rownames=F, type='html')
print(xtable(summarised.data.gendered[order(summarised.data.gendered$Driver, summarised.data.gendered$UAS),], caption="Gendered and Cross Level Summarised Data", label="tab:gendered_data", digits = c(rep(0,9),5,5)), include.rownames=F, type='html')
print(xtable(all.progeny.padj, caption="Summarized Data Including Double Balancer Strains", label="tab:all_progeny_data", digits = c(0,5,5)), type='html')
```
...
```{r data-tables-2,echo=FALSE,results='asis'}
...
```{r 24b-figure, echo=FALSE, dev=c('png','pdf')}
...
```{r 24b-figure-18c, echo=FALSE, dev=c('png','pdf')}
...
```{r hand-figure, echo=FALSE, dev=c('png','pdf')}
...
```{r c179-figure, echo=FALSE, dev=c('png','pdf')}
...
To test how many generations we can get out of 10 female flies in a vial, we examined the number of births in a vial, vs the generation of that vial.  These data are in Figure \ref{fig:generations}.  There is no detectable decrease in fecundity over time (p=`r anova(generation.lm, drop1(generation.lm))[1,5]`).

On the other hand, there was a significant difference between the number of Male flies born vs the number of Female flies born (p=`r t.test(data$Total~data$Gender)$p.value`).  This is visulized in Figure \ref{fig:gender}
